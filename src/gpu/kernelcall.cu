...
void kernel_shared_conv_host(Rgb *device_img, Rgb *img, int width, int height, int r)
{
    ...
    {
        std::cout << "❌ <Strel_size> parameter must be between 1 and 16 due to shared memory constaint.\n"
                  << std::endl;
...
void kernel_conv_host(Rgb *device_img, Rgb *img, int width, int height, int conv_size)
{
    if (conv_size <= 0)
    {
        std::cout << "❌ <Conv_size> parameter must be strictly greater than 0.\n"
                  << std::endl;
        assert(conv_size > 0);
        return;
    }
...
void kernel_pixelize_host(Rgb *device_img, Rgb *img, int width, int height, int pix_size)
{
    ...
    {
        std::cout << "❌ <Pix_size> parameter must be between 2 and 32 included.\n"
                  << std::endl;
        assert(pix_size > 1 and pix_size < 33);
        return;
    }

    std::cout << "📦 Creating gpu unit grid" << std::endl;
